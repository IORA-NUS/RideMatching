...
public class compromiseMatching {
	...
	public compromiseMatching(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
	...
	public void doMatching(){
		...
		System.out.println("revenueweight:"+weightRevenue+"pickuptimeweight"+weightPickupTime+"serviceweight"+weightServiceScore);
		...
		try {
			...
			GRBModel compModel = new GRBModel(env);
			compModel.set(GRB.StringAttr.ModelName, "Compromise Matching");
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = compModel.addVar(0, 1.0, weightRevenue*p.getRevenue() + weightPickupTime*(reverseParameter - reducedPickupTime[i][j])/2 + weightServiceScore*d.getServiceScore(), GRB.BINARY, "x" + i + j);
					}
					else {
						x[i][j] = compModel.addVar(0, 0.0, 1.0, GRB.BINARY, "x" + i + j);
					}
				}
			}
			...
			compModel.set(GRB.IntAttr.ModelSense, -1);
			...
			for (int i = 0; i < paxList.size(); ++i){
				...
				compModel.addConstr(expr_link, GRB.LESS_EQUAL, 1.0, "expr_link"+i);
			}
			...
			for (int j = 0; j < driverListPotential.size(); ++j){
				...
				compModel.addConstr(expr_link_2, GRB.LESS_EQUAL, 1.0, "expr_link_2"+j);
			}
