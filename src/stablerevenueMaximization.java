...
public class stablerevenueMaximization {
	...
	public stablerevenueMaximization(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = stablerevmaxModel.addVar(0, 1.0, p.getRevenue(), GRB.BINARY, "x" + i + j);
					}
					else {
