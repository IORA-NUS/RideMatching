...
public class compromiseMatching2 {
	...
	public compromiseMatching2(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = compModel.addVar(0, 1.0, weightRevenue*p.getRevenue() + weightPickupTime*(reverseParameter - reducedPickupTime[i][j])/2, GRB.BINARY, "x" + i + j);
					}
					else {
