...
public class weightedMatching {
	...
	public weightedMatching(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
	...
	public void doMatching(){
		...
		try {
			...
			GRBModel weightModel = new GRBModel(env);
			weightModel.set(GRB.StringAttr.ModelName, "Weighted Matching");
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = weightModel.addVar(0, 1.0, p.getRevenue() + (reverseParameter - reducedPickupTime[i][j])/2 + d.getServiceScore(), GRB.BINARY, "x" + i + j);
					}
					else {
						x[i][j] = weightModel.addVar(0, 0.0, 1.0, GRB.BINARY, "x" + i + j);
					}
				}
			}
			...
			// The objective is to maximize static weighted obj
			weightModel.set(GRB.IntAttr.ModelSense, -1);
			...
			for (int i = 0; i < paxList.size(); ++i){
				...
				weightModel.addConstr(expr_link, GRB.LESS_EQUAL, 1.0, "expr_link"+i);
			}
			...
			for (int j = 0; j < driverListPotential.size(); ++j){
				...
				weightModel.addConstr(expr_link_2, GRB.LESS_EQUAL, 1.0, "expr_link_2"+j);
			}

			// Solve
			weightModel.optimize();
