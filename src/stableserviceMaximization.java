...
public class stableserviceMaximization {
	...
	public stableserviceMaximization(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
	...
	public void doMatching(){
		...
		// Use gurobi to solve the service maximization problem with stability constraints
		try {
			GRBEnv env = new GRBEnv();
			GRBModel stableservmaxModel = new GRBModel(env);
			stableservmaxModel.set(GRB.StringAttr.ModelName, "Service Maximization Matching with Stability Constraints");
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = stableservmaxModel.addVar(0, 1.0, d.getServiceScore(), GRB.BINARY, "x" + i + j);
					}
					else {
						x[i][j] = stableservmaxModel.addVar(0, 0.0, 1.0, GRB.BINARY, "x" + i + j);
					}
				}
			}
			...
			// The objective is to maximize the service
			stableservmaxModel.set(GRB.IntAttr.ModelSense, -1);
			// Update model to integrate new variables
			stableservmaxModel.update();		       
			...
			for (int i = 0; i < paxList.size(); ++i){
				...
				stableservmaxModel.addConstr(expr_link, GRB.LESS_EQUAL, 1.0, "expr_link"+i);
			}
			...
			for (int j = 0; j < driverListPotential.size(); ++j){
				...
				stableservmaxModel.addConstr(expr_link_2, GRB.LESS_EQUAL, 1.0, "expr_link_2"+j);
			}
		          ...
		          for (int j = 0; j < driverListPotential.size(); ++j){
		        	   if (reducedPickupTime[i][j] < maxAllowedPickupTime){
		        	       ...
		        	       for (int jj = 0; jj < driverListPotential.size(); ++ jj){
		        		        if (reducedPickupTime[i][jj] <= reducedPickupTime[i][j]){
		        			        expr_preference.addTerm(1.0, x[i][jj]);
		        		        }
		        	        }
		        	  		expr_preference.addTerm(1.0, x[i][j]);  
		        	  		stableservmaxModel.addConstr(expr_preference, GRB.GREATER_EQUAL, 1.0, "expr_preference"+i+j);
                        }
		          }
              }
