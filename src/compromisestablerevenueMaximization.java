...
public class compromisestablerevenueMaximization {
	...
	public compromisestablerevenueMaximization(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
		          ...
		          for (int j = 0; j < driverListPotential.size(); ++j){
		        	  if (reducedPickupTime[i][j] < maxAllowedPickupTime){
                         ...
                         for (int ii = 0; ii < paxList.size(); ++ ii){
		        		     if (reducedPickupTime[ii][j] <= reducedPickupTime[i][j]){
