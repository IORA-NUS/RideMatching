...
public class pickuptimeMinimization {
	...
	public pickuptimeMinimization(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
	...
	public void doMatching(){
		...
		try {
			...
			GRBModel pickminModel = new GRBModel(env);
			pickminModel.set(GRB.StringAttr.ModelName, "Pickup Time Minimization Matching");
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = pickminModel.addVar(0, 1.0, (reverseParameter - reducedPickupTime[i][j])/2, GRB.BINARY, "x" + i + j);
					}
					else {
						x[i][j] = pickminModel.addVar(0, 0.0, 1.0, GRB.BINARY, "x" + i + j);
					}
				}
			}
			...
			pickminModel.set(GRB.IntAttr.ModelSense, -1);
			...
			for (int i = 0; i < paxList.size(); ++i){
				...
				pickminModel.addConstr(expr_link, GRB.LESS_EQUAL, 1.0, "expr_link"+i);
			}
			...
			for (int j = 0; j < driverListPotential.size(); ++j){
				...
				pickminModel.addConstr(expr_link_2, GRB.LESS_EQUAL, 1.0, "expr_link_2"+j);
			}

			// Solve
			pickminModel.optimize();
