...
public class compromisestableserviceMaximization {
	...
	public compromisestableserviceMaximization(ArrayList<Passenger> paxList, ArrayList<Driver> driverList, int distanceTimeConversion, int maxAllowedPickupTime, double weightRevenue, double weightPickupTime, double weightServiceScore, double reverseParameter) {
		super();
	...
	public void doMatching(){
		...
		try {
			...
			GRBModel stablerevmaxModel = new GRBModel(env);
			stablerevmaxModel.set(GRB.StringAttr.ModelName, "Revenue Maximization Matching with Stability Constraints");
			...
			for (int j = 0; j < driverListPotential.size(); j++) {
				...
				for (int i = 0; i < paxList.size(); i++){
					...
					if (reducedPickupTime[i][j] < maxAllowedPickupTime){
						x[i][j] = stablerevmaxModel.addVar(0, 1.0, weightRevenue*p.getRevenue() + weightPickupTime*(reverseParameter - reducedPickupTime[i][j])/2 + weightServiceScore*d.getServiceScore(), GRB.BINARY, "x" + i + j);
					}
					else {
						x[i][j] = stablerevmaxModel.addVar(0, 0.0, 1.0, GRB.BINARY, "x" + i + j);
					}
				}
			}
			...
			stablerevmaxModel.set(GRB.IntAttr.ModelSense, -1);
			// Update model to integrate new variables
			stablerevmaxModel.update();		       
			...
			for (int i = 0; i < paxList.size(); ++i){
				...
				stablerevmaxModel.addConstr(expr_link, GRB.LESS_EQUAL, 1.0, "expr_link"+i);
			}
			...
			for (int j = 0; j < driverListPotential.size(); ++j){
				...
				stablerevmaxModel.addConstr(expr_link_2, GRB.LESS_EQUAL, 1.0, "expr_link_2"+j);
			}
		          ...
		          for (int j = 0; j < driverListPotential.size(); ++j){
		        	  if (reducedPickupTime[i][j] < maxAllowedPickupTime){
		        	     GRBLinExpr expr_preference = new GRBLinExpr();
		        	  	 ...
		        	  	 stablerevmaxModel.addConstr(expr_preference, GRB.GREATER_EQUAL, 1.0, "expr_preference"+i+j);
                      }
		          }
